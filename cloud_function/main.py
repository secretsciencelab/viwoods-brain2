...
def process_note_to_markdown(note_path, output_path, existing_md_path=None, service=None, parent_folder_id=None, note_name=None, is_daily=False):
    ...
    prompt = "Transcribe the handwritten notes in this document into clean, structured Markdown. You MUST format every note with exactly 3 elements in this strict order: 1. A Title formatted as a Markdown H1 heading (e.g., `# Better Mousetrap`) on the first line. 2. One or more hashtags (e.g., `#idea #design`) on the second line. 3. The Body of the note. If the user did not explicitly write tags at the top, first search the body of the text for any hashtags and use those. If there are no tags anywhere, extract or infer one or more from the Title. CRITICAL RULES: 1. NEVER insert a space between the '#' symbol and the tag word (e.g. use `#tagname` NOT `# tagname`). Headers MUST have a space (e.g. `# Header Name`). Pay incredibly close attention to words starting with `#` in the handwriting (e.g. `#maxim`, `#todo`), and ALWAYS transcribe them exactly as hashtags, never mistaking them for headers. 2. If you see a hand-drawn empty square box next to a sentence, format it as a Markdown checkbox `- [ ]` (or `- [x]` if checked). 3. If you see a vertical line or bracket in the margin grouping multiple paragraphs together, wrap all those paragraphs in a Markdown blockquote (prefix lines with `> `) and include any hashtag written next to the bracket inside the block. 4. If you see a drawn horizontal line across the page, format it exactly as a Markdown horizontal rule (`---`) to act as a section break. 5. If you see a hand-drawn diagram (like a flowchart, wireframe, mindmap, or architecture graph), analyze it and convert it into a clean, scalable SVG code block (wrapped in ```xml). Make sure the SVG is self-contained with a viewBox, responsive, and styled cleanly with modern aesthetics (e.g., #0f172a, #3b82f6). For simple doodles or sketches that are NOT structured diagrams, write a highly detailed visual description enclosed in brackets: `[Drawing: A detailed description of what the sketch depicts]`."
