    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                system_instruction="You are a strict markdown transcriber. CRITICAL RULE: When transcribing or inferring hashtags, you MUST exclusively use the prefix 'HASHTAG_word'. NEVER use the '#' symbol for tags. The '#' symbol is strictly reserved for Markdown headings only."
            )
        )
        if response.text:
            ...
            # Convert HASHTAG_word safely back to #word
            raw_text = response.text.strip()
            # Also catch if Gemini put spaces after HASHTAG_ (e.g. HASHTAG_ Running)
            processed_text = re.sub(r'HASHTAG_\s*([a-zA-Z0-9_/-]+)', r'#\1', raw_text, flags=re.IGNORECASE)
